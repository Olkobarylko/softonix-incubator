<template>
  <div class="max-w-[450px]">
    <el-select
      v-model="selectValue" class="mb-5" multiple filterable allow-create default-first-option
    ...
    <p class="mb-2">
      Showing <span class="font-bold">{{ sum }}</span> out of <span class="font-bold">{{
        jobOpeningsOther().length }}</span> job openings
    </p>
    <div class="w-[100%] h-[1px] bg-[#c9c9c9] mb-5" />
    <ul v-for="(item, index) in filterJobs" :key="index" class="custom-list">
      <h2 class="font-bold mb-4 pl-4">{{ item.department }} ({{ item.filterArray.length }})</h2>
      <li v-for="(job, index) in item.filterArray" :key="index" class="text-light-blue pl-10 mb-2">
        {{ Object.values(job)[0] }}
      </li>
